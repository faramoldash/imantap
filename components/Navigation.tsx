...
const Navigation: React.FC<NavigationProps> = ({ currentView, setView, language }) => {
  ...
  const items: { id: ViewType; icon: string; label: string }[] = [
    ...
    { id: 'ai-insights', icon: '✨', label: t.navInsights },
    { id: 'useful-materials', icon: '📚', label: t.navUseful },
  ];

  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-t border-slate-100 flex justify-around items-center h-20 px-2 pb-4 z-50 shadow-[0_-5px_20px_rgba(0,0,0,0.05)]">
      {items.map((item) => (
        <button
          key={item.id}
